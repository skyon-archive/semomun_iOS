...
final class MyPurchasesVC: UIViewController {
    ...
    #if DEBUG
    private let viewModel = PayHistoryVM(onlyPurchaseHistory: true, networkUsecase: MockPayNetworkUsecase())
    #else
    private let viewModel = PayHistoryVM(onlyPurchaseHistory: true, networkUsecase: NetworkUsecase(network: Network()))
    #endif
...
extension MyPurchasesVC: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let item = self.viewModel.purchaseOfEachMonth[indexPath.section].content[indexPath.row]
...
extension MyPurchasesVC: UIScrollViewDelegate {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if (self.purchaseList.contentOffset.y + self.purchaseList.frame.size.height) > self.purchaseList.contentSize.height {
